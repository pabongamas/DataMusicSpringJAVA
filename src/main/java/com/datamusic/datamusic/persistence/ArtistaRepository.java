...
import com.datamusic.datamusic.persistence.pageableAndSort.ArtistaPagSortRepository;
...
public class ArtistaRepository implements ArtistRepository {
  ...
  @Autowired
  private ArtistaPagSortRepository artistaPagSortRepository;
  ...
  public Page<Artist> getAllPageable(Pageable pageable) {
    Page<Artista> artistaAllPageable = artistaPagSortRepository.findAll(pageable);
    List<Artista> artistaListAll = artistaAllPageable.getContent();
    List<Artist> artistListAll = mapper.toArtists(artistaListAll);
    return new PageImpl<>(artistListAll, pageable, artistaAllPageable.getTotalElements());
  }
